...
\thanks{We are grateful to Ignacio Fernández for exceptional research assistance. %, to XXX and YYY for in depth discussions related to data, and to ZZZ for comments and suggestions. [CONAF??]
We gratefully acknowledge support of the Inter American Development Bank, through the project ``Implications of Climate Change and Natural Disasters in LAC''. We thank participants and organizers of a number of workshops at the Inter American Development Bank for many productive suggestions. Clarke acknowledges partial support from the Institute for Research in Market Imperfections and Public Policy, MIPP.}} %(ICS13\_002 ANID).}}
...
\begin{spacing}{1.2}
\begin{abstract} 
Wildfires are increasing in frequency and intensity.  We study the impact of exposure to wildfires on human health, educational outcomes and labour market outcomes, in both the short, medium and long term. Using 30 years of administrative data from Chile combined with comprehensive records of wildfire from 1991-2020 we seek to isolate both the impacts of immediate exposure to wildfire, as well as understand dynamic impacts owing to exposure at a particularly sensitive periods.  Our administrative records allow us to follow individuals in a granular way over life, and even to follow individuals across generations.  In order to isolate the effect of exposure to wildfires we combine two-way fixed effect models with local changes in wind direction.  This paper seeks to provide a more complete account of the costs of exposure to wildfires.  This is a preliminary document for submission to BID focusing principally on data and proposed methods, as well as initial descriptive results.
\end{abstract}
...
%That is, those individuals that are equally likely to be exposed to wildfire air pollution due to their proximity to burnable lands, are differentially exposed to the air pollution from these wildfires due the quasi-randomness introduced by the exact location of the wildfire together the wind direction at the time of the wildfire.
...
\begin{figure}
    \centering
    \includegraphics[scale=0.6]{./results/descriptives/fireSizes.pdf}
...
\begin{figure}[htpb!]
    ...
    \begin{subfigure}{0.45\textwidth}
          \includegraphics[scale=0.46]{./results/descriptives/firesHA0_10.eps}
         \caption{$<$10 Ha}
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
          \includegraphics[scale=0.46]{./results/descriptives/firesHA10_100.eps}
         \caption{[10-100) Ha}
    \end{subfigure}

    \begin{subfigure}{0.45\textwidth}
          \includegraphics[scale=0.46]{./results/descriptives/firesHA100_500.eps}
         \caption{[100-500) Ha}
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
          \includegraphics[scale=0.46]{./results/descriptives/firesHA500_1000.eps}
         \caption{[500-1000) Ha}
    \end{subfigure}
...
To capture meteorological and climate conditions we use reanalysis data from ERA5's Copernicus Satellite Sentinels provided by ECMWF. This allows us to obtain ground level data for temperature, precipitations and wind direction at the 0.1° x 0.1°(equivalent to roughly 9 x 9 Km.). %We complement this with data from meteorological stations from Chile's Meteorological Service Agency and the Waters directorate of Chile’s Ministry for Agriculture.
...
\section{Methods}
\label{scn:methods}
%We estimate fixed-effect and differences-in-differences regressions to analyze the expected effects of climate-related wildfires on human health and economic activity. 	 

%We propose to econometrically examine the relationship between exposure to wildfires in the prenatal period, early in life, and later in life on health at birth, all cause morbidity and mortality, and specific classes of morbidity and mortality, such as respiratory distress, as well as the static and dynamic impacts of these exposures on educational and labour market outcomes. We propose to use dynamic two-way fixed effect models such as those proposed in de Chaisemartin \& D’Haultfoueille (2020), as well as recent synthetic difference-in-difference models based on staggered adoption designs (Arkhangelsky et al., 2021) which allow us to both consider immediate impacts, and dynamic impacts of exposure to wildfire which accumulate potentially long after initial exposure. Such models are based on parallel trend assumptions that outcomes in areas affected and unaffected by wildfires at a specific moment of time would have followed parallel paths if it were not for the fire. We can partially evaluate such assumptions by examining full pre-event trends in affected and non-affected areas.  We propose to use the full power of recent advances in observational techniques based on natural experiments and two-way fixed effect models, and additionally adequately account for inference in a setting where multiple outcomes are considered following Romano \& Wolf (2005).

% EXPLICAR QUE SE VA A HACER UN ANALISIS ESTATICO, A NIVEL COMUNA-SEMANA, 
% Y UN ANALISIS DYNAMICO A NIVEL DE SEGUIMIENTO DE LOS INDIVIDUOS, EN EL QUE SE LE HACE SEGUIMIENTO A 6.6 MILLONES DE PERSONAS
...
% CRT Note: Commenting below because it will not make in into the IDB-wp version
...
%However it does allow us to identify the immediate impacts of exposure to fire at other ages.  For this reason, we propose a second design, which allows us to estimate the impacts of exposure to wildfires at all ages based on a long panel of municipal by week cells.  
...
Given this, and to ensure that we capture \emph{exogenous} exposure to wildfire, we follow the empirical strategy developed by \cite{RangelVogl2019} and also employed by \cite{zivin2020unintended} to identify the causal effects of wildfires. %This will be conducted once we have processed our data on wind direction (both, from Copernicus satellite data as well as several ground level meteorological stations). 
...
\begin{figure}[htpb!]
    \centering
    \vspace{1mm}
    \includegraphics[scale=0.50]{./results/descriptives/DownwindFigure_2.png}
...
% CRT NOTE: I WOULD MOVE THE PRESENTATION AND ANALYSIS OF figure \ref{fig:design} TO THE APPENDIX. I THINK THAT THE KEY FIGURE THAT EXPLAINS THE DESIGN IS Figure \ref{fig:design2}
...
%\textcolor{red}{Edit the following for clarity.  Add link to Rangel \& Vogl, etc.}
...
\textcolor{red}{CURRENTLY UNDER REVISION BY CRT: 
To address the second comment from IDB reviewer we need to better motivate these results for PM$_{2.5}$ and explain how they relate to the main results below.}
\begin{figure}[htpb!]
    \caption{Exposure to Wildfires and PM$_{2.5}$}
    \label{fig:firstStage}
    \centering
    \includegraphics{./results/figures/exposurePM25.pdf}
    \floatfoot{Notes: Each coefficient and confidence interval presents the impact of regression where airborne PM 2.5 concentrations are regressed on the number of wildfires which are \textit{upwind} (red solid line) and \textit{downwind} (blue dashed line) from a given municipality.  Observations consist of municipality cells in each 6 hour block occurring from 2002 to 2021 (19,xxx,xxx observations).  Each specification includes full municipal and hour$\times$day$\times$month$\times$year fixed effects, and the definition of the independent variable is based on exposure to wildfires of the size indicated on the x-axis or generated. 95\% confidence intervals are presented based on standard errors clustered by municipality.}
\end{figure}

%\subsection{Descriptive Analysis}
%Add here a table of descriptive statistics as well as descriptive graphs and/or figures that introduce and motivate the most interesting relationships of our data.

%CAMBIOS A LA TABLA DE ESTADISTICAS DESCRIPTIVAS:
%1) Cambiar comas por puntos, y viceversa.
%2) Poner el número de incendios una sola vez (tal vez al final de la tabla solamente?
%3) Cambiar las etiquetas de los paneles: Panel A - Birth Outcomes; Panel B - Hospital Admissions; Panel C - Mortality Outcomes
%\begin{figure}[htpb!]
%    \centering
%    \includegraphics[scale=0.55]{./Descriptive/Tab1.png}
%    \caption{Caption}
%    \label{fig:my_label}
%\end{figure}
...
%EN ESTA ETAPA (PARA EL PRIMER BORRADOR), TENEMOS QUE CITAR LA LITERATURA QUE MUESTRA EFECTOS NO SIGNIFICATIVOS DEL ANALISIS DE EFECTOS FIJOS . QUE ESTO ES COMPLETAMENTE ESPERABLE A ESTAS ALTURAS DE ANALISIS

%ESTAMOS METIENDO EN EL MISMO SACO LOS 'TREATED' CON LOS 'NON-TREATED', POR LO QUE ES DE ESPERAR QUE EL EFECTO DEL INCENDIO PRACTIAMENTE QUE SE CANCELE COMPLETAMENTE.

%DE HECHO, LOS PAPERS EN LA LITERATURA EXPLICAN QUE ESTOS NO-EFECTOS SON LOS QUE HACE QUE SEA MUHO MAS IMPORTANTE EL ENFOCARSE EN EL TEMA DE EXPOSICION A LA CONTAMINACION PRODUCTO DE LA DIRECCION DEL VIENTO.
...
\begin{figure}[htpb!]
    ...
    \caption{Fires $\geq$ 500 Ha}
    \end{subfigure}        
    \floatfoot{Notes: Refer to notes to Figure     \label{fig:hospRF_byAge_upwind}.  All details are identical in this figure, however here exposure refers to non-upwind exposure to fires in the week and municipality cell.}
\end{figure}
...
\begin{figure}[htpb!]
    ...
    \caption{Burns}
    \end{subfigure}    
    \floatfoot{Notes: Each coefficient and set of confidence intervals is drawn from a separate regression model identical to that presented in Table \ref{tab:RFhosp} based on municipality and week cells from 2004--2019.  Outcomes are defined as mortality per 100,000 in the age group indicated on the horizontal axis.  Each panel is based on dependent variable (upwind fire exposure) to any wildfire of 140 hectares are greater.  All other details follow notes to Table \ref{tab:RFhosp}.}
\end{figure}
...
%\textcolor{red}{CRT COMMENT: IN THIS SECTION I'M FOCUSING ONLY ON VERY LARGE AND MEGA WILDFIRES. From the results we obtained for the IDB presentation, those wildfire sizes turned out to have significant effects on Infant Hospitalizations.}
...
\begin{figure}[htpb!]
    ...
    \centering
     ...
     \end{subfigure}    
       ...
       \floatfoot{Notes: Each coefficient and set of confidence intervals is drawn from a separate regression model akin to that presented in Table \ref{tab:RFhosp} based on municipality and week cells from 2004--2019. These estimates are obtained from running a dynamic model, as in equation (\ref{eq:TwoWayDynamic}), with 6 lags and 4 leads}
\end{figure}
...
\begin{figure}[htpb!]
    ...
    \begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[width=.99\textwidth]{./results/figures/rate_resp_00_01_200.pdf}
    \caption{Infants}
    \end{subfigure}    
    \begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[width=.99\textwidth]{./results/figures/rate_resp_01_05_200.pdf}
    \caption{Toddlers}
    \end{subfigure}    
    \begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[width=.99\textwidth]{./results/figures/rate_resp_06_15_200.pdf}
    \caption{Children}
    \end{subfigure}    
    \begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[width=.99\textwidth]{./results/figures/rate_resp_16_40_200.pdf}
    \caption{Young Adults}
    \end{subfigure}
    \begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[width=.99\textwidth]{./results/figures/rate_resp_41_65_200.pdf}
    \caption{Middle Age}
    \end{subfigure}    
    \begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[width=.99\textwidth]{./results/figures/rate_resp_65_pl_200.pdf}
    \caption{Elderly}
    \end{subfigure}  
    \floatfoot{Notes: }
\end{figure}
...
\begin{figure}[htpb!]
    ...
    \label{fig:mortAll}
    \includegraphics{./results/figures/mort_all_250.pdf}
    \end{figure}
...
\begin{figure}
    ...
    \label{fig:mortResp}
    \includegraphics{./results/figures/mort_resp_250.pdf}
\end{figure}
...
\begin{figure}
    ...
    \label{fig:mortCirc}
    \includegraphics{./results/figures/mort_circ_250.pdf}
\end{figure}
...
\begin{figure}
    ...
    \label{fig:mortCirc}
    \includegraphics{./results/figures/mort_burn_250.pdf}
\end{figure}
...
%Currently, we are downloading and processing Wind direction data to identify \textit{downwind} vs \textit{upwind} municipalities for each period of time $t$. This will be followed by a similar analysis that will download and process satellite-level PM$_{2.5}$ data. This polluton data will be used to run a first-stage type analysis of the effect of wildfire $\times$ wind-direction on PM$_{2.5}$ pollution concentrations, per municipality $m$ and for each period of time $t$.

%The \textit{dynamic} analysis remains to be conducted. This should follow \cite{Clarkeetal2022} by identifying those individuals that were impacted by wildfire air pollution while in the womb or early in life, and examine their health outcomes later in life.

%On the other hand, we understand that the drivers of wildfires are different from those of cropfires. Whereas the former is likely to be associated to unintentional ignition and fueled by arid land cover (which can be linked high temperatures and prevalence of droughts), the latter is usually a much more controlled fire that is part of a common agricultural soil management practice. To address this issue, we plan on using data on land cover layers from CONAF as well as satellite-level data from Google Earth Engine.\footnote{Alternative, a proxy identification of these two types of fires could be conducted by identifying those fires that are small and short-lasting, as these tend to be associated to cropfires rather than wildfires}
